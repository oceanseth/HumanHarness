...
class Memory {
  ...
  async connect() {
    ...
    let host = "unknown";
    try { host = new URL(this.config.url).hostname; } catch {}

    try {
      const { FalkorDB } = require("falkordb");

      const connect = FalkorDB.connect({ url: this.config.url });
      const timeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error("connection timed out (instance may be provisioning or sleeping)")), CONNECT_TIMEOUT_MS),
      );

      // Losing the race must not leak the client: close it if it lands late.
      const racing = Promise.race([connect, timeout]);
      racing.catch(() => connect.then((db) => db.close()).catch(() => {}));

      const db = await racing;
